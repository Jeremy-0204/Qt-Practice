...
void SquircleRenderer::paint()
{
    ...
    // Optionally, reset specific OpenGL state settings if required
    glDisable(GL_BLEND);
    glEnable(GL_DEPTH_TEST);
    // Reset viewport, if necessary: glViewport(x, y, width, height);

    // Flush the current OpenGL state, if needed: glFlush();
}
//! [5]
